...
test("public wrapper licences are explicit and do not relicense the server", () => {
  ...
  const plugin = read("wordpress-plugin/agentready-woo/agentready-woo.php");
  const pluginReadme = read("wordpress-plugin/agentready-woo/readme.txt");
  ...
  assert.match(publicReadme, /`wordpress-plugin\/agentready-woo\/\*\*` is licensed GPL-2\.0-or-later/);
