...
const path = fileURLToPath(new URL(
  "../wordpress-plugin/utilityhouse-release-gate-for-woocommerce/blueprints/blueprint.json",
  import.meta.url,
));
...
test("the Playground demo is pinned, local-first and activation-safe", () => {
  ...
  assert.equal(blueprint.landingPage, "/wp-admin/admin.php?page=utilityhouse-release-gate-for-woocommerce");
  ...
  assert.deepEqual(plugin, {
    ...
    path: "wordpress-plugin/utilityhouse-release-gate-for-woocommerce",
    ref: "wordpress-v1.2.1",
    refType: "tag",
  });
