...
class UtilityHouse_Release_Gate {
	...
	public static function release_gate_evidence_envelope( $store_id, $key, $nonce, $checks, $key_id = 'current' ) {
		...
		$now = gmdate( 'c' );
		$body = array( 'schema_version' => '2026-09-06', 'store_id' => $store_id, 'generated_at' => $now, 'expires_at' => gmdate( 'c', time() + 300 ), 'nonce' => $nonce, 'key_id' => $key_id, 'collector_version' => UTILITYHOUSE_RELEASE_GATE_VERSION, 'families' => array( 'woo' ), 'checks' => $checks );
	...
	public static function submit_release_gate_evidence( $endpoint, $store_id, $derived_key, $nonce, $checks, $key_id = 'current' ) {
		...
		if ( ! function_exists( 'wp_remote_post' ) || ! $origin ) {
			return new WP_Error( 'utilityhouse_release_gate_evidence_unavailable' );
		}
		...
		return wp_remote_post( $origin . '/api/v2/release-evidence', array(
			...
			'headers' => array(
				...
				'X-UtilityHouse-Release-Gate-Signature' => $envelope['signature'],
			),
			'body' => wp_json_encode( $envelope['evidence'], JSON_UNESCAPED_SLASHES ),
		) );
	}

	const OPTION_WORKER_URL = 'utilityhouse_release_gate_service_url';
	const OPTION_OWNERSHIP_KEY = 'utilityhouse_release_gate_ownership_key';
	const OPTION_RELEASE_STORE_ID = 'utilityhouse_release_gate_store_id';
	const OPTION_RELEASE_CURRENT_KEY = 'utilityhouse_release_gate_evidence_current';
	const OPTION_RELEASE_PREVIOUS_KEY = 'utilityhouse_release_gate_evidence_previous';
	const OPTION_RELEASE_KEY_ID = 'utilityhouse_release_gate_evidence_key_id';
	const OPTION_RELEASE_ENABLED = 'utilityhouse_release_gate_evidence_enabled';
	const CRON_RELEASE_EVIDENCE = 'utilityhouse_release_gate_evidence';
	...
	public static function init() {
		...
		add_action( 'admin_post_utilityhouse_release_gate_evidence', array( __CLASS__, 'admin_submit_release_evidence' ) );
	...
	public static function add_rewrite() {
		add_rewrite_rule( '^\.well-known/agenticweb\.md$', 'index.php?utilityhouse_release_gate_discovery=1', 'top' );
		add_rewrite_rule( '^\.well-known/utilityhouse-release-gate-ownership$', 'index.php?utilityhouse_release_gate_ownership=1', 'top' );
	}

	public static function query_vars( $vars ) {
		$vars[] = 'utilityhouse_release_gate_discovery';
		$vars[] = 'utilityhouse_release_gate_ownership';
		return $vars;
	}
	...
	public static function maybe_serve_discovery( $wp = null ) {
		...
		$ownership = ! empty( $query_vars['utilityhouse_release_gate_ownership'] ) || ( function_exists( 'get_query_var' ) && get_query_var( 'utilityhouse_release_gate_ownership' ) );
		$discovery = ! empty( $query_vars['utilityhouse_release_gate_discovery'] ) || ( function_exists( 'get_query_var' ) && get_query_var( 'utilityhouse_release_gate_discovery' ) );
		if ( $ownership ) {
	...
	public static function discovery_markdown() {
		...
		$lines[] = '';
		$lines[] = '## UtilityHouse Release Gate';
		$lines[] = '';
		$lines[] = 'This plugin publishes a local discovery record. It does not claim that a store or release passed UtilityHouse checks.';
		$lines[] = '';
		$lines[] = '- public Store API: ' . esc_url( rest_url( 'wc/store/v1/products' ) );
	...
	public static function robots_txt( $output, $public ) {
		...
		$lines  = array( '', '# UtilityHouse Release Gate: AI shopping agents are welcome' );
	...
	public static function register_settings() {
		register_setting( 'utilityhouse_release_gate', self::OPTION_WORKER_URL, array( 'type' => 'string', 'sanitize_callback' => array( __CLASS__, 'sanitize_worker_origin' ) ) );
		register_setting( 'utilityhouse_release_gate', self::OPTION_OWNERSHIP_KEY, array( 'type' => 'string', 'sanitize_callback' => function( $v ) { return self::sanitize_release_key( $v, self::OPTION_OWNERSHIP_KEY ); } ) );
		register_setting( 'utilityhouse_release_gate', self::OPTION_RELEASE_STORE_ID, array( 'type' => 'string', 'sanitize_callback' => array( __CLASS__, 'sanitize_release_id' ) ) );
		register_setting( 'utilityhouse_release_gate', self::OPTION_RELEASE_CURRENT_KEY, array( 'type' => 'string', 'sanitize_callback' => function( $v ) { return self::sanitize_release_key( $v, self::OPTION_RELEASE_CURRENT_KEY ); } ) );
		register_setting( 'utilityhouse_release_gate', self::OPTION_RELEASE_PREVIOUS_KEY, array( 'type' => 'string', 'sanitize_callback' => function( $v ) { return self::sanitize_release_key( $v, self::OPTION_RELEASE_PREVIOUS_KEY ); } ) );
		register_setting( 'utilityhouse_release_gate', self::OPTION_RELEASE_KEY_ID, array( 'type' => 'string', 'sanitize_callback' => function( $v ) { return $v === 'previous' ? 'previous' : 'current'; } ) );
		register_setting( 'utilityhouse_release_gate', self::OPTION_RELEASE_ENABLED, array( 'type' => 'string', 'sanitize_callback' => function( $v ) { return $v === '1' ? '1' : '0'; } ) );
	}
	...
	public static function admin_submit_release_evidence() { if ( ! current_user_can( 'manage_woocommerce' ) || ! check_admin_referer( 'utilityhouse_release_gate_evidence' ) ) wp_die( 'forbidden' ); $sent = self::run_release_evidence( true ); wp_safe_redirect( add_query_arg( 'utilityhouse_evidence', $sent ? 'sent' : 'failed', admin_url( 'admin.php?page=utilityhouse-release-gate-for-woocommerce' ) ) ); exit; }
	...
	public static function local_readiness_snapshot() {
		...
		return array(
			...
			'utilityhouse_store_abilities' => 'NONE_REGISTERED',
	...
	public static function settings_page() {
		...
		// phpcs:ignore WordPress.Security.NonceVerification.Recommended
		$evidence_notice = isset( $_GET['utilityhouse_evidence'] ) ? sanitize_key( wp_unslash( $_GET['utilityhouse_evidence'] ) ) : '';
		?>
		<div class="wrap">
			<h1>UtilityHouse Release Gate</h1>
			...
			<?php if ( $evidence_notice ) : ?><div class="notice notice-<?php echo $evidence_notice === 'sent' ? 'success' : 'error'; ?>"><p><?php echo $evidence_notice === 'sent' ? esc_html__( 'Aggregate evidence sent.', 'utilityhouse-release-gate-for-woocommerce' ) : esc_html__( 'Evidence was not sent. Check the HTTPS endpoint, store id and derived key.', 'utilityhouse-release-gate-for-woocommerce' ); ?></p></div><?php endif; ?>
			...
			<form method="post" action="options.php">
				<?php settings_fields( 'utilityhouse_release_gate' ); ?>
				<table class="form-table" role="presentation">
					<tr>
						<th scope="row"><label for="uhrg-worker">UtilityHouse endpoint</label></th>
						<td>
							<input name="<?php echo esc_attr( self::OPTION_WORKER_URL ); ?>" id="uhrg-worker" type="url" class="regular-text code"
								placeholder="https://app.utilityhouse.xyz" value="<?php echo esc_attr( $worker ); ?>" />
							<p class="description">The HTTPS UtilityHouse service endpoint from your authenticated connection bundle. Saving this alone sends nothing.</p>
						</td>
					</tr>
					<tr><th scope="row"><label for="uhrg-release-store">Release Gate store id</label></th><td><input name="<?php echo esc_attr( self::OPTION_RELEASE_STORE_ID ); ?>" id="uhrg-release-store" type="text" class="regular-text code" value="<?php echo esc_attr( $release_store ); ?>" /><p class="description">The opaque store id from your authenticated UtilityHouse connection bundle.</p></td></tr>
					<tr><th scope="row"><label for="uhrg-release-current">Release Gate derived keys</label></th><td><input name="<?php echo esc_attr( self::OPTION_RELEASE_CURRENT_KEY ); ?>" id="uhrg-release-current" type="password" class="regular-text code" autocomplete="new-password" value="" placeholder="Current key (leave blank to keep)" /><br /><input name="<?php echo esc_attr( self::OPTION_RELEASE_PREVIOUS_KEY ); ?>" type="password" class="regular-text code" autocomplete="new-password" value="" placeholder="Previous key during rotation (optional)" /><p class="description">These per-store derived keys are never displayed after save and are used only for short-lived evidence signatures.</p></td></tr>
					<tr><th scope="row"><label for="uhrg-release-key-id">Active derived key</label></th><td><select name="<?php echo esc_attr( self::OPTION_RELEASE_KEY_ID ); ?>" id="uhrg-release-key-id"><option value="current" <?php selected( $release_key_id, 'current' ); ?>>Current</option><option value="previous" <?php selected( $release_key_id, 'previous' ); ?>>Previous</option></select></td></tr>
					...
					<tr>
						<th scope="row"><label for="uhrg-release-key">Release Gate ownership key</label></th>
						<td><input name="<?php echo esc_attr( self::OPTION_OWNERSHIP_KEY ); ?>" id="uhrg-release-key" type="password" class="regular-text code" autocomplete="new-password" value="" placeholder="Ownership key (leave blank to keep)" />
							<p class="description">Paste the 64-character per-store key from your authenticated UtilityHouse connection bundle. It is never displayed after save and is used only to HMAC short-lived ownership challenges.</p></td>
					</tr>
				</table>
				<?php submit_button(); ?>
			</form>
			<h2>Release Gate evidence</h2>
			<p>The button is explicit one-time consent to contact the configured UtilityHouse service. It checks only whether a published product exists and never transmits product details, customers, orders, payments, or credentials.</p>
			<form method="post" action="<?php echo esc_url( admin_url( 'admin-post.php' ) ); ?>"><input type="hidden" name="action" value="utilityhouse_release_gate_evidence" /><?php wp_nonce_field( 'utilityhouse_release_gate_evidence' ); submit_button( 'Send Release Gate evidence now', 'secondary', 'submit', false ); ?></form>
